...
export default class FineCalculatorService {
  private static calculate(slip: any, insertedDate: number, dueDate: number) {
  ...
  public static async fineCalculator(id: string) {
    try {
      const slip = await BankSlipDao.findById(id);
